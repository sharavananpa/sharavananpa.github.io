...
function fnv1aHash(string) {
    const FNV_PRIME = 0x01000193; // FNV prime for 32-bit hash
    const OFFSET_BASIS = 0x811C9DC5; // FNV offset basis for 32-bit hash

    let hash = OFFSET_BASIS;
    for (let i = 0; i < string.length; i++) {
        hash ^= string.charCodeAt(i); // XOR with byte value
        hash = (hash * FNV_PRIME) >>> 0; // Multiply by FNV prime and ensure 32-bit
    }
    return hash.toString(16); // Convert to hex for easier use
}
...
function changeFavicon(color) {
    ...
    favicon.href = blobCache.get(hash);
}
